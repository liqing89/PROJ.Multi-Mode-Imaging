...
class imaging:
    ...
    def Azimuthwindow(self):
        ...
        window = np.fft.fftshift(window).reshape(-1,1)
        return window
    def imgSave(self, mode, imgPath, tiffPath):
        '''
        截取成像区域并显示
        '''
    ...
    def Calculate_PSLR(self,ss,dim):
        ...
        # PSLR = np.sort(scipy.signal.find_peaks(prof)[0])[::-1]
        # PSLR = PSLR[1]
        # if dim == 0:
        #     prof = image_abs[:,maxid]
        # else:
        #     prof = image_abs[maxid,:]
        # peakid = np.argmax(prof)
        # ransideid = np.where(prof<0)[0]
        # sideidsort = np.sort(np.abs(ransideid-peakid))
        # sidepeakid = sideidsort[:2]
        # sideid1 = np.where(np.abs(ransideid-peakid)==sidepeakid[0])[0][0]
        # sideid2 = np.where(np.abs(ransideid-peakid)==sidepeakid[1])[0][-1]
        # sideid1,sideid2 = sideid2,sideid1
        # mainlobe = np.sum(prof[ransideid[sideid1]:ransideid[sideid2]]**2)
        # ISLR = 10*np.log10((np.sum(prof**2)-mainlobe)/np.sum(prof**2))
        # return PSLR,ISLR
        # return {
        #     ""
        # }

    def dataExpose(self, mode = 2):
        # self.evalInfo = {
        #     "azmInfo": self.Calculate_PSLR(self.image, 0),
        #     "rngInfo": self.Calculate_PSLR(self.image, 1)
        # }
        '''
        截取成像区域并显示
        '''
        ...
        # real_part = I.real.astype(np.float32)
        # imag_part = I.imag.astype(np.float32)
        # tif_image = np.stack((real_part,imag_part),axis=0)
